...
import { payrollRuns, payrollRunSteps, employees, payslips, } from '../db/schema.js';
...
route.get('/', authMiddleware, async (c) => {
    ...
    const totalResult = where
        ? await db.select({ count: sql `count(*)::int` }).from(payrollRuns).where(where)
        : await db.select({ count: sql `count(*)::int` }).from(payrollRuns);
...
route.post('/', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        if (periodMonth == null || periodYear == null)
            return c.json({ error: 'periodMonth and periodYear required' }, 400);
        ...
        if (existing.length > 0)
            return c.json({ error: 'Payroll run for this period already exists' }, 409);
...
route.put('/:id/step', authMiddleware, async (c) => {
    ...
    if (!stepDef)
        return c.json({ error: 'Invalid stepName' }, 400);
    const [stepRow] = await db
        .select()
        .from(payrollRunSteps)
        .where(and(eq(payrollRunSteps.payrollRunId, id), eq(payrollRunSteps.stepName, stepName)))
        .limit(1);
    if (stepRow?.completedAt)
        return c.json({ error: 'Step already completed' }, 409);
    const statusMap = {
    ...
    const nextStatus = stepName === 'report_uploaded' ? 'audit_pending'
