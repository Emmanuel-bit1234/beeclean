...
import { employees, budgets, payrollRuns, employeeVerifications, ministries, payslips, messages, } from '../db/schema.js';
...
route.get('/', authMiddleware, async (c) => {
    try {
        ...
        const spentStatuses = ['payment_done', 'reconciled'];
        const spentRuns = await db
            .select({ budgetTotal: payrollRuns.budgetTotal })
            .from(payrollRuns)
            .where(and(eq(payrollRuns.periodMonth, currentMonth), eq(payrollRuns.periodYear, currentYear), inArray(payrollRuns.status, spentStatuses)));
        const totalSpent = spentRuns.reduce((sum, r) => sum + Number(r.budgetTotal ?? 0), 0);
        ...
        for (const m of ministryList) {
            ...
            const budgetRow = await db
                .select({ amount: budgets.amount })
                .from(budgets)
                .where(and(eq(budgets.ministryId, m.id), eq(budgets.periodMonth, currentMonth), eq(budgets.periodYear, currentYear)))
                .limit(1);
