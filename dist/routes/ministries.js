...
route.get('/', authMiddleware, async (c) => {
    try {
        ...
        if (paymentDay) {
            ...
            if (!isNaN(day))
                result = result.filter((m) => m.paymentDayOfMonth === day);
        }
        if (search) {
...
route.post('/', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        if (day < 1 || day > 31)
            return c.json({ error: 'paymentDayOfMonth must be 1-31' }, 400);
...
route.put('/:id', authMiddleware, adminMiddleware, async (c) => {
    ...
    if (body.sectorCategory != null)
        update.sectorCategory = String(body.sectorCategory).trim();
