...
route.get('/', authMiddleware, async (c) => {
    ...
    if (ministryId) {
        ...
        if (!isNaN(mid))
            conditions.push(eq(budgets.ministryId, mid));
    }
    if (departmentId) {
        const did = parseInt(departmentId, 10);
        if (!isNaN(did))
            conditions.push(eq(budgets.departmentId, did));
    }
    if (periodMonth) {
        const m = parseInt(periodMonth, 10);
        if (!isNaN(m))
            conditions.push(eq(budgets.periodMonth, m));
    }
    if (periodYear) {
        const y = parseInt(periodYear, 10);
        if (!isNaN(y))
            conditions.push(eq(budgets.periodYear, y));
    }
