...
route.get('/', authMiddleware, async (c) => {
    ...
    if (ministryId) {
        ...
        if (!isNaN(mid))
            conditions.push(eq(excelUploads.ministryId, mid));
    }
    if (uploadType)
        conditions.push(eq(excelUploads.uploadType, uploadType));
    if (status)
        conditions.push(eq(excelUploads.status, status));
...
route.post('/', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        if (!fileName || !uploadType)
            return c.json({ error: 'fileName and uploadType required' }, 400);
