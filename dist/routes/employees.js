...
route.get('/', authMiddleware, async (c) => {
    try {
        ...
        if (departmentId) {
            ...
            if (!isNaN(did))
                conditions.push(eq(employees.departmentId, did));
        }
        ...
        if (search) {
            const term = `%${search}%`;
            conditions.push(or(like(employees.name, term), like(employees.surname, term), like(employees.employeeNumber, term), like(employees.position, term)));
        }
 ...
 */
async function generateEmployeeNumber(ministryId) {
...
route.post('/', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        const { ministryId, departmentId, employeeNumber: bodyEmployeeNumber, name, surname, email, position, salary, bankAccount, bankName, mobileMoneyProvider, mobileMoneyNumber, } = body;
...
route.put('/:id', authMiddleware, adminMiddleware, async (c) => {
    ...
    for (const key of allowed) {
        if (body[key] !== undefined) {
            if (key === 'status' && !['active', 'suspended', 'deceased', 'retired'].includes(body[key]))
                continue;
            if (key === 'mobileMoneyProvider' && !['mpesa', 'airtel_money', 'orange_money', 'none'].includes(body[key]))
                continue;
