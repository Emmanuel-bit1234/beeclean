...
usersRoute.get('/search', authMiddleware, async (c) => {
    try {
        ...
        const conditions = [
            or(like(users.name, searchTerm), like(users.surname, searchTerm), like(users.email, searchTerm)),
        ];
...
usersRoute.get('/', authMiddleware, async (c) => {
    try {
        ...
        if (search) {
            ...
            conditions.push(or(like(users.name, searchTerm), like(users.surname, searchTerm), like(users.email, searchTerm)));
        }
        const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
        // Get total count
        const totalResult = await db
...
usersRoute.put('/:id', authMiddleware, async (c) => {
    try {
        ...
        if (body.surname !== undefined)
            updateData.surname = body.surname;
        if (body.email)
