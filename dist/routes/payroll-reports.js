...
const REPORT_TYPES = ['monthly', 'audit', 'authorisation', 'payment', 'reconciliation'];
...
route.get('/', authMiddleware, async (c) => {
    ...
    if (payrollRunId) {
        ...
        if (!isNaN(rid))
            conditions.push(eq(payrollReports.payrollRunId, rid));
    }
    if (ministryId) {
        const mid = parseInt(ministryId, 10);
        if (!isNaN(mid))
            conditions.push(eq(payrollReports.ministryId, mid));
    }
    if (periodMonth) {
        const m = parseInt(periodMonth, 10);
        if (!isNaN(m))
            conditions.push(eq(payrollReports.periodMonth, m));
    }
    if (periodYear) {
        const y = parseInt(periodYear, 10);
        if (!isNaN(y))
            conditions.push(eq(payrollReports.periodYear, y));
    }
    if (reportType && REPORT_TYPES.includes(reportType)) {
        conditions.push(eq(payrollReports.reportType, reportType));
    }
